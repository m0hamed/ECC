...
class BasicLinearCode:
  ...
  def codewords(self):
    if self._codewords is None:
      self._codewords = []
      # messages are generated by the cartesian product of the set possible elements of the base field k times
      # where k is the rank/dimension of the code (the size of the messages).
      for msg in itertools.product(*[self._base_ring.list() for _ in xrange(self._rank)]):
        self._codewords.append(self.encode(self.get_vector(msg)))
  ...
  # returns the decoded codeword using either mode="nearest" or mode="syndrome"
  def decode(self, word, mode="nearest"):
    if mode == "nearest":
      return self.nearest_neighbour(word)
    elif mode == "syndrome":
      return self.syndrome_coset_leader_correction(word)
    else:
      return None
